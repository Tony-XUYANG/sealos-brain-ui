...
test('deploy form runs the agent workflow', async ({ page }) => {
  ...
  await expect(page.locator('.nav-cta .rocket-icon')).toHaveCount(1);
  await expect(button.locator('.rocket-icon')).toBeVisible();
  const rocketMask = await button.locator('.rocket-icon').evaluate((icon) => {
    const style = getComputedStyle(icon);
    return style.maskImage || style.webkitMaskImage;
  });
  expect(rocketMask).toContain('data:image/svg+xml');
